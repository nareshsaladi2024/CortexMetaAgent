...
if (-not $env:MCP_REASONING_COST_URL) {
    Write-Host "Using default ReasoningCost MCP server URL: http://localhost:8002" -ForegroundColor Cyan
    Write-Host "   Set `$env:MCP_REASONING_COST_URL to use a different URL" -ForegroundColor Gray
    Write-Host ""
}
