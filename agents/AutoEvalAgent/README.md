...
- **MCP Integration**: Uses AgentInventory MCP to list agents and TokenStats MCP for token validation
...
2. **Negative Set** (600 examples)
   ...
   - Token-limit overflow prompts (using TokenStats MCP)
...
1. **MCP Servers**: 
   ...
   - TokenStats MCP: `http://localhost:8000` (for token validation)
