...
if (-not $env:MCP_AGENT_INVENTORY_URL) {
    Write-Host "Using default AgentInventory MCP server URL: http://localhost:8001" -ForegroundColor Cyan
    Write-Host "   Set `$env:MCP_AGENT_INVENTORY_URL to use a different URL" -ForegroundColor Gray
    Write-Host ""
}
